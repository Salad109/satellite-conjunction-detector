...
for i, (ratio, df) in enumerate(datasets):
    ax1.plot(df['tolerance_km'], df['coarse_s'], 'o-', linewidth=2, markersize=8, label=f'Ratio {ratio}')
    ax2.plot(df['tolerance_km'], df['refine_s'], 's-', linewidth=2, markersize=8, label=f'Ratio {ratio}')
