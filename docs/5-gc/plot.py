...
for f in csv_files:
    ...
    df = pd.read_csv(f)
    dfs[name] = df
    avgs.append((name, df.mean(numeric_only=True)))
...
stack_colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#06A77D', '#e377c2', '#17becf', '#9467bd', '#D62839', '#8c564b']
stack_labels = ['Pair Reduction', 'Filter', 'Propagator Build', 'SGP4', 'Interpolation', 'Check Pairs', 'Grouping', 'Refine', 'Probability']
