...
for i, (stride, data) in enumerate(datasets):
    ax.plot(data['tolerance_km'], data['total_s'], 'o-', linewidth=2, markersize=8, label=f'Stride {stride}')
...
# Plot 2 - Stacked area per stride
...
for i, (stride, data) in enumerate(datasets):
    ax1.plot(data['tolerance_km'], data['conj'], 'o-', linewidth=2, markersize=8, label=f'Stride {stride}')
